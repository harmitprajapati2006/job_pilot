...
| AI model                       | Groq (llama-3.3-70b-versatile) | Matching, research synthesis, extraction         |
...
├── agent/
│   ├── adzuna.ts                          → Adzuna API job discovery + Groq scoring
...
Adzuna API returns job listings
        ↓
Groq (llama-3.3-70b-versatile) scores each job against user profile
        ↓
...
Navigates to company homepage + sub pages
        ↓
Groq (llama-3.3-70b-versatile) synthesizes dossier from extracted content
        ↓
...
API route in app/api/resume/
        ↓
Groq (llama-3.3-70b-versatile) processes content
        ↓
...
| match_reason       | text        | Groq explanation                               |
...
try {
  ...
  // Log and continue — Groq will synthesize from what was found
...
- Company research always returns a dossier — even if browser research fails, Groq (llama-3.3-70b-versatile) synthesizes from company name and job description alone. Never return empty.
