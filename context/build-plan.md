...
Extract from Resume button — Groq (llama-3.3-70b-versatile) reads uploaded PDF and auto-fills profile form fields.
...
Generate a clean professional PDF resume from current profile data using Groq (llama-3.3-70b-versatile).

**Logic:**

- POST /api/resume/generate
- Reads current profile data from profiles table
- Groq generates professional resume content:
  - Professional summary paragraph
  - Polished work experience bullet points
  - Clean professional language throughout
- @react-pdf/renderer renders Groq output into clean single-page PDF using renderToBuffer()
...
- For each job returned:
  ...
  - Groq (llama-3.3-70b-versatile) scores job against user profile:
...
- AI Match Reasoning section — match reason paragraph from Groq
...
- Derive company homepage URL by following the Adzuna redirect with server-side fetch() — no browser needed for this step:
  ...
  - If Stagehand gets no meaningful content (oneLiner and productSummary empty) — skip browser research entirely, proceed to Groq synthesis with job description and profile only
...
- Close Browserbase session after homepage + max 3 sub-pages
  **Groq synthesis (runs after browser closes):**
...
- Always return a dossier — never fail silently. If browser research failed, Groq synthesizes from job description and profile alone.
